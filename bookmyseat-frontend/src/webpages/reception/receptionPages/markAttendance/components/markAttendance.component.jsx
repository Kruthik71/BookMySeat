...
const MarkAttendance = () => {
  ...
  const handleAttendance = (empId) => {
    ...
    const enteredOTP = otpMap[empId];
    const booking = employeeBookings.find(
      (booking) => booking.employeeId === empId
    );
    const backendOTP = booking ? booking.otp : null;
    // console.log("Selected EMployee: ", booking);
    ...
    // If OTPs are not matching
      ...
      setModalHeading("Data Does'nt Match");
      setModalMessage(
        "Entered OTP does not match the OTP received from the backend."
      );
      setShowModal(true);
    }
  };
