...
import { faCamera, faPenToSquare, faChevronRight} from '@fortawesome/free-solid-svg-icons';
...
export default function Profile() {
   ...
   //fetching profile details from backend through the api used in axios
    useEffect(()=> {
        ...
        axios.get('http://localhost:9006/bookmyseat/user/profile')
    ...
    const handleSavePhoneNumber = () => {
        ...
        axios.put('http://localhost:9006/bookmyseat/user/editprofile', {
    ...
    return (
        ...
        <div className={styles.profilePage}>
            ...
            <div className={styles.secondSection}>
                ...
                <FontAwesomeIcon icon={faPenToSquare} className={styles.pen} onClick={handleOpenPhoneModal} />
            </div>
            <div className={styles.profileComp}>
                <div className={styles.compForDetails}>Branch <p> Bengaluru </p></div>
                <div className={styles.compForDetails}>Email ID <p>{data?.email || ''} </p></div>
                <div className={styles.compForDetails}>Phone Number <p>{data?.phoneNumber || ''}</p></div>
                <div className={styles.compForDetails}>Employee ID <p>{data?.employeeId || ''}</p></div>
                <div className={styles.compForDetails}>Role <p>{data?.role?.roleName ||''}</p></div>
            </div>
                ...
                <div className={styles.modal}>
                    <div className={styles.modalContent}>
                        ...
                        <div className='btncontainer'>
                            <button className={styles.savePhoneno} onClick={handleSavePhoneNumber}>Save</button>
                            <button className={styles.cancelPhoneno} onClick={handleClosePhoneModal}>Cancel</button>
                        </div>
                    </div>
                </div>
            )}
