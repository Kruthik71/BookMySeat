...
import styles from "../styles/mezzanineFloorMap.module.css";
import { Laptop2 } from "lucide-react";
import axios from "axios";
import ResponseModal from "../../messageModal/responseModal";

export const MezzanineFloor = ({
  onSeatSelect,
  setMapContent,
  isAdmin,
  mapData,
  ...
  useEffect(() => {
    ...
    if (isAdmin) {
      try {
        axios
          .get("http://localhost:9006/bookmyseat/admin/seatrestriction/2")
          .then((res) => {
            const data = res.data;
            console.log(data);
  ...
  //extracting project id of logged in user
  const userProjectId = userInformation.userProjectId;
  ...
  const handleSeatClick = (seatNumber, seatIdCount) => {
    ...
    if (reservedProjectSeat) {
      if (userProjectId === reservedProjectSeat.projectId) {
        alert("allow him to reserve here");
      } else {
        // toast message here
        alert("Seat is Reserved for project ", reservedProjectSeat.projectName);
        // setModalHeading("Project Reserved Seat!");
        // setModalMessage(
  ...
  return (
    <div className={styles.modalContainer}>
      <div className={styles.mezzanineFloor}>
        <div className={styles.headingContainer}>
          <h1>Mezzanine Floor</h1>
        </div>
